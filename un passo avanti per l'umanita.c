...
void dijkstra(size_t, size_t, int pesi[*][*], int nodi);						 // calcolo
...
void percorso(int costi[], int dest, int part, int prec[], int nodi);			 // risale il percorso e lo stampa
...
void inserimento(size_t c, size_t r, int pesi[c][r], int nodi, int flag, int righe) // funzione per l'imput di una matrice nodi per nodi al momento statica
{
    ...
    case 1:
        for (i = 0; i < nodi; i++) // input orientato
        {
            ...
            pulisci();
            wprintf(L"\\");
            // aggiungere if che se i nodi sono più di 21 stampa a numeri
            for (cont = 0; cont < nodi; cont++)
            {
                wprintf(L"\t%c", lettere);
                lettere++;
...
void modifica(size_t c, size_t r, int pesi[c][r], int nodi, int flag){
	int mod;
	char part[1];
	char dest[1];
	int p, d;
