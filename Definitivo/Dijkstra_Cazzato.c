...
void dijkstra(FILE *fp, size_t c, size_t r, int pesi[c][r], int nodi)
{
    ...
    do
    {
        ...
        // input compreso tra 0 e 2
        do
        {
            wprintf(L"\nVuoi calcolare un altro percorso utilizzando la stessa matrice?\nDigita 0 per calcolare un altro percorso con la stessa matrice \nDigita 1 per modificare la matrice esistente e calcolare il percorso minimo \nDigita 2 per tornare al menu' principale\n");
            scanf("%d", &flag);
        } while (flag != 0 && flag != 1 && flag != 2);
        if (flag == 1)
        {
            output(c, pesi, nodi);
            printf("La matrice riportata di sopra è quella attuale\n");
...
int scelta(int righe)
{
    ...
    {
        wprintf(L"%*sDigita 1 per la soluzione di Default\n%*s Digita 2 per la soluzione con dati inseriti dall'utente\n", righe - 11, "", righe - 21, "");
        scanf("%d", &sceltaDEF);
